...
class GameStateNode:
    ...
    def evaluate(self):
        # Define direction set once based on agent color
        ...
        if self.color == PlayerColor.RED:
            ...
            dirs |= {Direction.Up, Direction.UpLeft, Direction.UpRight}

        # 1) Progress
        red_prog  = sum(f.r for f in self.red_frogs)
        blue_prog = sum(7 - f.r for f in self.blue_frogs)
        prog_score = (red_prog - blue_prog) if self.color==PlayerColor.RED else (blue_prog - red_prog)

        # 2) Mobility
        def mobility(frogs):
            mv = 0
            all_others = self.red_frogs | self.blue_frogs
            for f in frogs:
                # steps
                for d in dirs:
                    dest = apply_direction(f,d)
                    if dest and dest in self.lily_pads and dest not in all_others:
                        mv += 1
                # jumps
                mv += len(find_jumps(f, f, dirs, all_others, self.lily_pads, {f}, []))
            return mv
        my_frogs  = self.red_frogs if self.color==PlayerColor.RED else self.blue_frogs
        opp_frogs = self.blue_frogs if self.color==PlayerColor.RED else self.red_frogs
        mob_score = mobility(my_frogs) - mobility(opp_frogs)

        # 3) Jump-Potential
        def jump_potential(frogs):
            jp = 0
            all_others = self.red_frogs | self.blue_frogs
            for f in frogs:
                paths = find_jumps(f, f, dirs, all_others, self.lily_pads, {f}, [])
                jp += sum(1 for p in paths if len(p) > 1)
            return jp
        jp_score = jump_potential(my_frogs) - jump_potential(opp_frogs)

        # 4) Pad-Control
        def pad_control(frogs):
            pc = 0
            all_others = self.red_frogs | self.blue_frogs
            for f in frogs:
                for d in dirs:
                    over = apply_direction(f, d)
                    dest = apply_direction(over, d) if over else None
                    if over in all_others and dest in self.lily_pads and dest not in all_others:
                        pc += 1
            return pc
        pc_score = pad_control(my_frogs) - pad_control(opp_frogs)
        ...
        # now combine with tuned weights
        return (  1.0 * prog_score
                + 0.5 * mob_score
                + 0.8 * jp_score
                + 0.3 * pc_score
                + 0.2 * block_score )
